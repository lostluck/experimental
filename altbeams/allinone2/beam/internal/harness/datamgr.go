...
func (s *ScopedDataManager) open(ctx context.Context, port Port) (*DataChannel, error) {
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
...
func (m *DataChannelManager) closeInstruction(instID instructionID, ports []Port) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
type elementsChan struct {
	...
	ch chan Elements // must only be closed by the read loop
...
// makeChannel creates a channel of Elements. It expects to be called while c.mu is held.
...
func (c *DataChannel) read(ctx context.Context) {
	...
	// This way we avoid one allocation per receive.
	// But ultimately, the sub lists of Timers and Data and are going to be problems.
	...
	for {
		...
		if err != nil {
			...
			slog.DebugContext(ctx, "DataChannel.read", "channel", c.id, "error", err)
			return
		}
		...
		iterateElements(c, cache, &seenLast, msg.GetTimers(),
			func(elm *fnpb.Elements_Timers) Elements {
				return Elements{Timers: elm.GetTimers(), PtransformID: elm.GetTransformId(), TimerFamilyID: elm.GetTimerFamilyId()}
			})

		iterateElements(c, cache, &seenLast, msg.GetData(),
			func(elm *fnpb.Elements_Data) Elements {
				return Elements{Data: elm.GetData(), PtransformID: elm.GetTransformId()}
			})
...
func iterateElements[E dataEle](c *DataChannel, cache map[instructionID]*elementsChan, seenLast *[]clientID, elms []E, wrap func(E) Elements) {
	...
	w := &dataWriter{ch: c, id: id,
		...
		elmMsg: fnpb.Elements{
			Data: []*fnpb.Elements_Data{
				{
					InstructionId: string(id.instID),
...
func (w *dataWriter) send(msg *fnpb.Elements) error {
	if err := w.ch.client.Send(msg); err != nil {
		if err == io.EOF {
			slog.Warn("dataWriter EOF on send; fetching real error", "writer", w.id, "channel", w.ch.id)
			err = nil
			for err == nil {
		...
		slog.Warn("dataWriter error on send: %v", "writer", w.id, "channel", w.ch.id, "error", err)
...
func (w *dataWriter) Close() error {
	...
	w.elmMsg.Data[0].IsLast = true
	w.elmMsg.Data[0].Data = nil
	msg := &w.elmMsg
	// msg := &fnpb.Elements{
	// 	Data: []*fnpb.Elements_Data{
	// 		{
...
func (w *timerWriter) send(msg *fnpb.Elements) error {
	if err := w.ch.client.Send(msg); err != nil {
		if err == io.EOF {
			slog.Warn("timerWriter  EOF on send; fetching real error: %v", "writer", w.id, "channel", w.ch.id)
			err = nil
			for err == nil {
				// Per GRPC stream documentation, if there's an EOF, we must call Recv
				// until a non-nil error is returned, to ensure resources are cleaned up.
		...
		slog.Warn("timerWriter error on send: %v", "writer", w.id, "channel", w.ch.id, "error", err)
		w.ch.terminateStreamOnError(err)
		return err
	}
	return nil
