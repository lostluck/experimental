...
func TestElementChan(t *testing.T) {
	...
	drainAndSum := func(t *testing.T, elms <-chan Elements) (sum, count int) {
		t.Helper()
	...
	openChan := func(ctx context.Context, t *testing.T, c *DataChannel, timers ...string) <-chan Elements {
		t.Helper()
	...
	tests := []struct {
		name               string
		sequenceFn         func(context.Context, *testing.T, *fakeChanClient, *DataChannel) <-chan Elements
